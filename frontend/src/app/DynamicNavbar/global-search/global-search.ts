...
export class GlobalSearchComponent implements AfterViewInit, OnDestroy {
  ...
  onInput(event: Event): void {
    ...
    this.navbar.globalSearchQuery.set(val);
    this.resetIa();
  ...
  onEnterSearch(): void {
    ...
    if (this.aiEnabled && q.endsWith('?')) {
      this.navbar.globalSearchResults.set([]);
      this.callIa(q);
      return;
    }
    this.navbar.searchGlobal(q);
  }

  setSuggestion(text: string): void {
    this.navbar.globalSearchQuery.set(text);
    this.resetIa();
  ...
  private callIa(q: string): void {
    ...
    this.iaService.chat(q).then(res => {
      ...
      this.navbar.globalSearchQuery.set('');
  ...
  execute(result: GlobalSearchResult): void { this.navbar.executeSearchAction(result); }
