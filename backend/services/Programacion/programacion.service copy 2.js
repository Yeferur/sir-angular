...
async function generarPlanLogistico(fecha, idTour, maxOpciones = 5) {
  ...
  for (const combinacion of combinaciones) {
    ...
    // ✅ Ordenar las reservas dentro de cada bus por Posicion
    for (const bus of buses) {
      bus.reservas.sort((a, b) => a.Posicion - b.Posicion);
    }
  ...
  return {
    ...
    sugerencias,
    mensaje: sugerencias.length > 0 ? `Se encontraron ${sugerencias.length} soluciones óptimas.` : 'No se pudo generar un plan automático completo.'
  };
}


async function generarPlanConFlotaDefinida(fecha, tour, combinacionManual) {
